...
namespace MCsim
{
	...
	public:
	bool DSARP_enabled;
		Ramulator(T* spec, unsigned int ranks): MemoryDevice(ranks)
		{
		...
		float get_constraints(const string& parameter) 
		{
			...
			else if (parameter == "tRP") {
				param = channel->spec->speed_entry.nRP;
			}
			else if (parameter == "tRTP") {
				param = channel->spec->speed_entry.nRTP;
			}
			else if (parameter == "tWR") {
				param = channel->spec->speed_entry.nWR;
			}
			else {
				param = 0;
			}
