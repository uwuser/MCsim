...
namespace MCsim
{
	...
	public:
		CommandScheduler_RankReOrder(vector<CommandQueue *> &commandQueues, const map<unsigned int, bool> &requestorTable) : CommandScheduler(commandQueues, requestorTable)
		{
			rankIndex = 0;
			rank_num = 2;
		...
		BusPacket *commandSchedule()
		{
			...
			if (!newRound && newType)
			{
				if (bundlingType == BusPacketType::RD)
				{
					bundlingType = BusPacketType::WR;
				}
				else
				{
					bundlingType = BusPacketType::RD;
				}
			}
