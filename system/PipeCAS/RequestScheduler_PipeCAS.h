...
namespace MCsim
{
	...
	public:
		RequestScheduler_PipeCAS(std::vector<RequestQueue *> &requestQueues, std::vector<CommandQueue *> &commandQueues, const std::map<unsigned int, bool> &requestorTable) : RequestScheduler(requestQueues, commandQueues, requestorTable)
		{
			newRound = true;
		...
		void requestSchedule()
		{
			...
			{
				if (bundlingType == RequestType::DATA_WRITE)
				{
					bundlingType = RequestType::DATA_WRITE;
				}
				else
				{
					bundlingType = RequestType::DATA_WRITE;
