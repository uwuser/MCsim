...
namespace MCsim
{
	...
	public:
		CommandScheduler_PIPECAS(vector<CommandQueue *> &commandQueues, const map<unsigned int, bool> &requestorTable, unsigned int srtSlot) : CommandScheduler(commandQueues, requestorTable)
		{
			srtN_ACT = 0;
			srtNum_ACT = 0;
			actCounter = 0;
		...
		BusPacket *commandSchedule()
		{
			...
			if (!queueScheduled.empty())
			{
				...
				if (scheduledCommand == NULL)
				{ // Before reaching the SRT CAS Stop Point
					if (casCounter > 0)
					{
						schedule_SRT_CAS();
						if (scheduledCommand == NULL)
				...
				if (scheduledCommand == NULL)
				{ // Executing PRE during HRT Round
					schedule_SRT_PRE();
				}
			...
			if (queueScheduled.empty() && roundStart == false && scheduledCommand == NULL)
			{ // Not In a HRT Round
				if (srtRD == 10)
				{
					roundSRT = WR;
			...
			if (!fawCounter.empty())
			{
				for (unsigned int index = 0; index < fawCounter.size(); index++)
				{
					fawCounter[index]--;
				}
				if (fawCounter.front() == 0)
				{
					fawCounter.erase(fawCounter.begin());
				}
			}
