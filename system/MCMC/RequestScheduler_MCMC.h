...
namespace MCsim
{
	...
	public:
		RequestScheduler_MCMC(std::vector<RequestQueue *> &requestQueues, std::vector<CommandQueue *> &commandQueues, const std::map<unsigned int, bool> &requestorTable, int dataBus) : RequestScheduler(requestQueues, commandQueues, requestorTable)
		{
			slotCounter = 0;
			requestorIndex = 0;
			dataBusSize = dataBus;
