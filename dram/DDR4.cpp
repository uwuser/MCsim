...
void DDR4::init_lambda()
{
    ...
    lambda[int(Level::Bank)][int(Command::ACT_R)] = [] (DRAM<DDR4>* node, int id) {
        node->state = State::Opened;
        node->row_state[id] = State::Opened;};
    lambda[int(Level::Bank)][int(Command::ACT_W)] = [] (DRAM<DDR4>* node, int id) {
        node->state = State::Opened;
        node->row_state[id] = State::Opened;}; 
...
void DDR4::init_timing()
{
    ...
    t[int(Command::ACT_R)].push_back({Command::ACT_R, 1, s.nRRDS});
    t[int(Command::ACT_W)].push_back({Command::ACT_W, 1, s.nRRDS});
    t[int(Command::ACT_R)].push_back({Command::ACT_W, 1, s.nRRDS});
    t[int(Command::ACT_W)].push_back({Command::ACT_R, 1, s.nRRDS});
    ...
    t[int(Command::ACT_R)].push_back({Command::ACT_R, 1, s.nRRDL});
    t[int(Command::ACT_W)].push_back({Command::ACT_W, 1, s.nRRDL});
    t[int(Command::ACT_W)].push_back({Command::ACT_R, 1, s.nRRDL});
    t[int(Command::ACT_R)].push_back({Command::ACT_W, 1, s.nRRDL});
