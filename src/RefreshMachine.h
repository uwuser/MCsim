...
namespace MCsim
{
	...
	{
	public:
		RefreshMachine(const vector<CommandQueue *> &commandQueues, size_t ranks, size_t banks, size_t tREFI, size_t tRFCpb, size_t tRFCab, size_t tRFC, size_t tRCD) : commandQueue(commandQueues),
																																										ranks(ranks),
																																										banks(banks),
																																										tREFI(tREFI),
																																										tRFCpb(tRFCpb),
...

		{
			...
			isInterval = false;
			flag = false;
			refresh_mechanism = "none"; // none; distributed;  per-bank (use DSARP only when there exists a subarray in the device. DDR3 and DDR4 does not have per bank refreshes)
		}
		...
		bool reachInterval() // reaching the interval when using per bank refreshes
		{
			return isInterval;
		}
		bool busyBank(unsigned int r_bank)
		{
			return isRefreshingBank[r_bank]; // block the banks that are busy
		}
