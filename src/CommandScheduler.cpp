...
CommandScheduler::CommandScheduler(
	vector<CommandQueue *> &commandQueues, const map<unsigned int, bool> &requestorTable) : commandQueue(commandQueues),
	...
	if (refreshMachine->reachInterval())
	{
		...
		if (tempCmd != NULL)
		{
			if (isIssuableRefresh(tempCmd))
			{
				...
				{
					TRACE_CMD("TRACE-COMMAND:PRE"
							  << "\t" << clock << ":"
							  << "\t\tAddress: " << tempCmd->address << "\t\tBank: " << tempCmd->bank << "\t\tColumn: " << tempCmd->column << "\tRow: " << tempCmd->row);
				}
				else if (tempCmd->busPacketType == REF)
				{
					TRACE_CMD("TRACE-COMMAND:REF"
							  << "\t" << clock << ":"
							  << "\t\tAddress: " << tempCmd->address << "\t\tBank: " << tempCmd->bank << "\t\tColumn: " << tempCmd->column << "\tRow: " << tempCmd->row);
				}
				else if (tempCmd->busPacketType == REFPB)
				{
					TRACE_CMD("TRACE-COMMAND:REFPB"
							  << "\t" << clock << ":"
							  << "\t\tAddress: " << tempCmd->address << "\t\tBank: " << tempCmd->bank << "\t\tColumn: " << tempCmd->column << "\tRow: " << tempCmd->row);
				}
				else if (tempCmd->busPacketType == PREA)
				{
					TRACE_CMD("TRACE-COMMAND:PREA"
							  << "\t" << clock << ":"
							  << "\t\tAddress: " << tempCmd->address << "\t\tBank: " << tempCmd->bank << "\t\tColumn: " << tempCmd->column << "\tRow: " << tempCmd->row);
				}
	...
	if (commandQueue[index]->isPerRequestor())
	{
		...
		if (cmd->busPacketType == PRE)
		{
			if (cmd->address > 999999)
			{
				TRACE_CMD("TRACE-COMMAND:PRE"
						  << "\t" << clock << ":"
						  << "\t\tAddress: " << cmd->address << "\tBank: " << cmd->bank << "\t\tColumn: " << cmd->column << "\tRow: " << cmd->row);
			}
			else
			{
				TRACE_CMD("TRACE-COMMAND:PRE"
